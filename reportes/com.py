...
def exportar_excel_data_prep(df_resultados, config_turnos):
    ...
    for fecha in fechas_unicas:
        ...
        nombres_lpp = [n for n, r in getattr(database, 'LPP', {}).items() if any(date.fromisoformat(i) <= fecha_dt <= date.fromisoformat(f) for i, f in r)]
        fila_lpp[fecha] = "\n".join(nombres_lpp)
        nombres_lar = [n for n, r in getattr(database, 'LAR', {}).items() if any(date.fromisoformat(i) <= fecha_dt <= date.fromisoformat(f) for i, f in r)]
        fila_lar[fecha] = "\n".join(nombres_lar)
...
def calcular_metricas_fin_de_semana_com(df_resultados, df_personal, dias_del_bloque, feriados_indices, fecha_inicio):
    ...
    def calcular_para_guardias(guardias, fi_dt, ff_dt, f_set):
        ...
        feriados_t = sum(1 for g in guardias if g['fecha'] in f_set)
