...
from data import TURNOS_SEMANA, TURNOS_FINDE, FECHA_INICIO
import db as _db

def _get_licencias(): return _db.LAR, _db.LPP

def aplicar_reglas_blandas(modelo, turnos, df_personal, dias_del_bloque, feriados, offset_dia, num_semanas):
    ...
    modelo.Minimize(
        (brecha_ratio_finde * 1000) + # Prioridad alta a nivelar el ratio de FS
        (brecha_anual * 100) +
        (brecha_mensual * 50) +
        (brecha_seg * 100) +
        ((max_fl3 - min_fl3) * 500) +  # Nivelar FSL3 trabajados
        ((max_fl4 - min_fl4) * 500) +  # Nivelar FSL4 trabajados
        (sum(semanas_seg_totales) * -150) - 
        (sum(puntos_seguimiento) * 5) - 
        (sum(puntos_combo_finde) * 15) - 
        (sum(puntos_preferencias) * 300) +
        (sum(puntos_mix_horario) * 500) +
        (sum(puntos_inconsistencia) * 100)
    )
