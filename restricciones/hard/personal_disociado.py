"""restricciones/hard/personal_disociado.py — Evita que parejas de personal compartan el mismo turno o la misma franja horaria el mismo día.

Regla dura agnóstica parametrizada desde la base de datos mediante la clave 'parejas' en JSON.
Ejemplo de configuración: {"parejas": [["Empleado A", "Empleado B"]]}
"""
...
def apply(modelo, ctx) -> None:
    params = _re.resolver_parametros_regla(
        'PERSONAL_DISOCIADO', 'GLOBAL', ctx.fecha_inicio, ctx.reglas_servicio, {}, {}
    )
    ...
    for p1_name, p2_name in params.get('parejas', []):
        ...
        for d in range(ctx.dias):
            ...
            for _, turnos_f in franjas.items():
                ...
                if vars1 or vars2:
                    add_hard(modelo, ctx,
                             modelo.Add(sum(vars1) + sum(vars2) <= 1),
                             f"{p1_name}_{p2_name}_d{d}_disociado")
