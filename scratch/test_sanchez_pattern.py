...
print("Construyendo modelo con patrón forzado para Sánchez Reinoso...")
...
for dia, turno in patron.items():
    ...
    if var_key in turnos:
        ...
        print(f"Forzado {nombre_sanchez} el día {dia} ({fecha_inicio_dt + timedelta(days=dia)}) en {turno} == 1")
    else:
        print(f"ERROR: Variable no disponible {var_key}")
...
if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
    print("\n¡EL PATRÓN ES VIABLE!")
    # Mostrar guardias asignadas a Sánchez Reinoso para verificar
    for d in range(total_dias):
        td = "Finde_Feriado" if is_f(d, offset_dia, feriados_indices) else "Semana"
        for t in config_turnos.get(td, {}).keys():
            if (nombre_sanchez, d, t) in turnos and solver.Value(turnos[(nombre_sanchez, d, t)]) == 1:
                print(f"Día {d} ({fecha_inicio_dt + timedelta(days=d)}): {t}")
else:
    print("\n¡EL PATRÓN ES INVIABLE!")
