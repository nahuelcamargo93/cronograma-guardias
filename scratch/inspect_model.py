...
def main():
    ...
    # Let's inspect the model constraints that are not enforced by assumptions, 
    # or print the CpModelProto constraints to see if any are added directly.
    proto = modelo.Proto()
    print(f"Total variables: {len(proto.variables)}")
    print(f"Total constraints: {len(proto.constraints)}")
    
    # Check if there are constraints without enforcement literal
    no_enf = 0
    for i, c in enumerate(proto.constraints):
        if not c.enforcement_literal:
            no_enf += 1
            if no_enf <= 10:
                print(f"\nConstraint {i} (no enforcement):")
                try:
                    lin = c.linear
                    if lin.vars:
                        vars_str = []
                    ...
                    print(c)
    print(f"Total constraints without enforcement literal: {no_enf}")
