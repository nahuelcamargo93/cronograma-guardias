...
def _aplicar_min_dia_especifico_mes_soft(modelo, turnos_vars, empleados, turnos_dict, reglas_servicio, ajustes_reglas, dias_del_bloque, fecha_inicio_dt, penalizaciones_ad_hoc, servicio_id=1):
    ...
    for emp in empleados:
        ...
        min_dias_req = params.get('min_dias', 1)
        ...
        if servicio_id == 3:
            # Regla de cálculo dinámico para Área Médica UTI (Servicio 3)
            if k >= 4:
                min_dias_req = 1
            elif k == 3:
                min_dias_req = 1
        ...
        if vars_dia:
            min_real = min(min_dias_req, len(vars_dia))
            violation = modelo.NewIntVar(0, min_real, f'viol_dia_esp_{emp.nombre}_{dia_semana_target}')
            modelo.Add(violation >= min_real - sum(vars_dia))
            # Penalidad muy alta: 1.000.000 (un millón) por cada viernes no cubierto!
            # De esta forma, el solver prioriza al 100% que todos hagan al menos 1 viernes.
            penalizaciones_ad_hoc.append(violation * 1000000)
