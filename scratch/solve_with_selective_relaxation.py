...
# Let's test a scenario where we:
# 1. Enforce MAX_HORAS_MES_CALENDARIO for Barloa, Godoy, Quintero, Mora.
# 2. Enforce it for all other doctors, but allow we to ignore it for:
#    - Pregot Analia Mariana
#    - Zeballos Valeria Alejandra
#    - Navarro Suarez Gabriela Belén
#    (or any other specific doctors). Let's see if this is feasible!

ignore_dict = {
    "Pregot Analia Mariana": ['MAX_HORAS_MES_CALENDARIO'],
    "Zeballos Valeria Alejandra": ['MAX_HORAS_MES_CALENDARIO'],
    "Navarro Suarez Gabriela Belén": ['MAX_HORAS_MES_CALENDARIO']
}
...
print("Building model with selective MAX_HORAS relaxation...")
...
print(f"Status with Pregot, Zeballos, Navarro Suarez relaxed: {status_names.get(status, 'UNKNOWN')}")
...
if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
    ...
    for emp in empleados:
        ...
        print(f"Name: {emp.nombre:35} | Assigned: {emp_hours:3d} hs | Max: {actual_max:3d} {exceeded}")
