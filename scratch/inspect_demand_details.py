...
def inspect():
    ...
    print("\n=== DEMANDA AJUSTES ===")
    res = conn.execute("""
        SELECT da.fecha_inicio, da.fecha_fin, p.nombre, da.cantidad_min, da.cantidad_max, da.activo
    ...
    """).fetchall()
    for row in res:
        print(f"Range: {row[0]} to {row[1]} | Puesto: {row[2]:<12} | Min: {row[3]} | Max: {row[4]} | Activo: {row[5]}")
