...
if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
    print("\n=== Valor de las Assumptions de MANEJO_FINDES (FLR) ===")
    # Buscaremos las variables de assumptions en ctx.assumptions
    ass_dict = {name: b for name, b in ctx.assumptions}
    for name, b in sorted(ctx.assumptions):
        if "MANEJO_FINDES" in name and "flr" in name:
            val = solver.Value(b)
            print(f"  {name} = {val}")
else:
