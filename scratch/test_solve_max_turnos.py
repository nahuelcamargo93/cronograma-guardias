...
try:
    ...
    # 2. Inject MAX_TURNOS rule for Jefe/Coordinadores in memory
    for emp in empleados:
        if emp.rol in ('Jefe', 'Coordinador'):
            t_name = "Mañana_UTI" if emp.rol == "Jefe" or emp.nombre in ("Franco, Leandro", "Moyano, Fernando") else "Mañana_UCO"
            # Set MAX_TURNOS rule for this employee
            emp.reglas['MAX_TURNOS'] = [{
                "turno": t_name,
                "max_por_semana": 4
            }]
            print(f"Inyectando MAX_TURNOS para {emp.nombre}: {t_name} max 4/semana")
    ...
    if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        print("\n=== RESULTADOS SIMULADOS CON MAX_TURNOS = 4 (FINES DE SEMANA) ===")
        ...
        print("¡El modelo resultó INVIABLE al limitar a 4 turnos semanales!")
