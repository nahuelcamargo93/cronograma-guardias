...
print("Constraint 0:")
print(modelo.Proto().constraints[0])
print("Variables in Constraint 0:")
c = modelo.Proto().constraints[0]
if c.linear:
    print("Linear vars:", list(c.linear.vars))
if c.bool_or:
    print("BoolOr literals:", list(c.bool_or.literals))
if c.bool_and:
    print("BoolAnd literals:", list(c.bool_and.literals))
if c.enforcement_literal:
    print("Enforcement literals:", list(c.enforcement_literal))

# Print what variable names those indexes correspond to
for idx in list(c.linear.vars) + list(c.bool_or.literals) + list(c.bool_and.literals) + list(c.enforcement_literal):
    var_name = modelo.Proto().variables[idx].name
    print(f"Index {idx}: {var_name}")
