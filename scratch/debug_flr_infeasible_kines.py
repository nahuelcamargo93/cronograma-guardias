...
# Investigar a 'Camargo, Nahuel', 'Coniglio, Melisa', 'Giacoppo, Veronica'
targets = ['Camargo, Nahuel', 'Coniglio, Melisa', 'Giacoppo, Veronica']

for target_name in targets:
    print(f"\n=========================================\nDIAGNOSING: {target_name}\n=========================================")
    # Filtrar empleados para dejar solo al target
    emp_filtered = [e for e in empleados if e.nombre == target_name]
    if not emp_filtered:
        print(f"Employee {target_name} not found!")
        continue
    
    # Construir el modelo solo para este empleado (usando assumptions)
    ...
    if status == cp_model.INFEASIBLE:
        print(f"Model is INFEASIBLE for {target_name}.")
        reportar_conflicto(solver, ctx)
    else:
        print(f"Model is FEASIBLE (status={status}) when running alone.")
