...
def test_individual():
    ...
    with get_connection() as conn:
        conn.execute("UPDATE servicios_reglas SET activo = 1 WHERE servicio_id = 3 AND codigo_regla = 'MIN_TURNOS_SEMANA'")
        conn.execute("UPDATE servicios_reglas SET activo = 0 WHERE servicio_id = 3 AND codigo_regla = 'MIN_FRANCOS_SEMANA'")
    ...
    print("\n>>> Probando optimización con SOLO MIN_TURNOS_SEMANA...")
    res1 = main.ejecutar_optimizacion(3, "2026-07-01", "2026-07-31", notas="Test solo min turnos", modo_debug=False, max_time_in_seconds=30)
    print(f"Resultado SOLO MIN_TURNOS_SEMANA: {res1}")

    # Caso 2: Solo MIN_FRANCOS_SEMANA activa
    with get_connection() as conn:
        conn.execute("UPDATE servicios_reglas SET activo = 0 WHERE servicio_id = 3 AND codigo_regla = 'MIN_TURNOS_SEMANA'")
        conn.execute("UPDATE servicios_reglas SET activo = 1 WHERE servicio_id = 3 AND codigo_regla = 'MIN_FRANCOS_SEMANA'")
    ...
    print("\n>>> Probando optimización con SOLO MIN_FRANCOS_SEMANA...")
    res2 = main.ejecutar_optimizacion(3, "2026-07-01", "2026-07-31", notas="Test solo min francos", modo_debug=False, max_time_in_seconds=30)
