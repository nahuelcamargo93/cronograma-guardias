...
def main():
    ...
    francos_forzados = db_queries.cargar_francos_forzados_rango(fecha_inicio, fecha_fin, servicio_id)
    asig_fijas_eventuales, asig_fijas_recurrentes = db_queries.cargar_asignaciones_fijas_rango(fecha_inicio, fecha_fin, servicio_id)
    
    offset_dia = fecha_inicio_dt.weekday()

    print(f"Construyendo modelo debug-soft del {fecha_inicio} al {fecha_fin}...")
    ...
    modelo, turnos, flr_tracker, ctx = construir_modelo(
        ...
        asignaciones_fijas_eventuales=asig_fijas_eventuales,
        asignaciones_fijas_recurrentes=asig_fijas_recurrentes,
        francos_forzados=francos_forzados
    )

    from restricciones.cargador import aplicar_objetivo_debug
    aplicar_objetivo_debug(modelo, ctx)
    ...
    if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        print("\n--- INFRACCIONES DE REGLAS HARD DETECTADAS ---")
        infracciones_hard = []
