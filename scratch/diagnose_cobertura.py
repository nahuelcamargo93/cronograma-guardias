...
for f_str in FERIADOS:
    ...
    delta = (f_dt - datetime.strptime(fecha_inicio, "%Y-%m-%d")).days
    if 0 <= delta < 30:
        feriados_indices.append(delta)

# We will write a custom function that replaces _aplicar_cobertura_dinamica
# but lets us skip a specific day or shift.
        ...
        for (h_start, h_end), window_demands in demandas_por_ventana.items():
            ...
            for dem in window_demands:
                ...
                for emp in empleados:
                    ...
                    es_extra = False
                    if _re_exists(params_extra) and isinstance(params_extra, dict):
...
def test_model(day_to_skip=None, shift_to_skip=None):
    ...
    # Replicate main.py construir_modelo variables definition
    turnos = {}
    ...
    # Now run hard rules except custom coverage
    # We will temporarily mock _aplicar_cobertura_dinamica in hard_rules
    import rule_engine as _re
    ...
    vars_turno_sem = _crear_y_vincular_variables_semanales(modelo, turnos, empleados, 30, fecha_inicio_dt_d, historial_semana_previa, offset_dia)
    
    _aplicar_licencias(modelo, turnos, empleados, config_turnos, offset_dia, feriados_indices)
    _aplicar_franco_forzado(modelo, turnos, empleados, 30, fecha_inicio_dt_d, config_turnos, reglas_servicio_db, ajustes_reglas)
    _aplicar_max_turnos(modelo, turnos, empleados, _get_semanas(30, fecha_inicio_dt_d), reglas_servicio_db, ajustes_reglas, historial_semana_previa, 30, fecha_inicio_dt_d)
    _aplicar_excluir_turnos(modelo, turnos, empleados, 30, offset_dia, fecha_inicio_dt_d, reglas_servicio_db, ajustes_reglas)
    _aplicar_min_turnos(modelo, turnos, empleados, _get_semanas(30, fecha_inicio_dt_d), reglas_servicio_db, ajustes_reglas, historial_semana_previa)
    
    # Call custom coverage
    ...
    _aplicar_limite_horas_semanales(modelo, turnos, empleados, _get_semanas(30, fecha_inicio_dt_d), reglas_servicio_db, ajustes_reglas, historial_semana_previa, config_turnos, turnos_dict, offset_dia, feriados_indices, limite_horas_global)
    _aplicar_descanso_entre_turnos(modelo, turnos, empleados, 30, fecha_inicio_dt_d, reglas_servicio_db, ajustes_reglas, offset_dia, feriados_indices, config_turnos, turnos_dict, historial_semana_previa)
    _aplicar_min_findes_mes(modelo, turnos, empleados, config_turnos, offset_dia, feriados_indices, reglas_servicio_db, ajustes_reglas, 30, 4)
    _aplicar_findes_completos_y_medios(modelo, turnos, empleados, config_turnos, offset_dia, feriados_indices, reglas_servicio_db, ajustes_reglas, 30)
    _aplicar_un_solo_turno_por_dia(modelo, turnos, empleados, 30, offset_dia, feriados_indices, fecha_inicio_dt_d, config_turnos, reglas_servicio_db, ajustes_reglas)
    _aplicar_max_horas_mes_calendario(modelo, turnos, empleados, 30, offset_dia, feriados_indices, fecha_inicio_dt_d, config_turnos, turnos_dict, reglas_servicio_db, ajustes_reglas)
    _aplicar_fin_licencia(modelo, turnos, empleados, 30, offset_dia, feriados_indices, config_turnos, reglas_servicio_db, ajustes_reglas, fecha_inicio_dt_d)
    _aplicar_min_horas_mes_calendario(modelo, turnos, empleados, 30, offset_dia, feriados_indices, fecha_inicio_dt_d, config_turnos, turnos_dict, reglas_servicio_db, ajustes_reglas)
    _aplicar_reglas_fechas_especiales(modelo, turnos, empleados, 30, fecha_inicio_dt_d, config_turnos, reglas_servicio_db, ajustes_reglas)
    _aplicar_balance_dia_noche(modelo, turnos, empleados, 30, offset_dia, feriados_indices, config_turnos, turnos_dict, reglas_servicio_db, ajustes_reglas, fecha_inicio_dt_d)
    _aplicar_personal_asociado(modelo, turnos, empleados, 30, offset_dia, feriados_indices, config_turnos, turnos_dict, reglas_servicio_db, ajustes_reglas)
...
def _get_semanas(dias_del_bloque, fecha_inicio_dt):
    semanas = {}
...
for shift in ["00:00", "06:00", "12:00", "18:00"]:
