...
# Replace multipliers in constraints
code = code.replace("* 1000", "* 100")

# Write to a temp file
os.makedirs("scratch", exist_ok=True)
with open("scratch/soft_rules_scaled.py", "w", encoding="utf-8") as f:
    f.write(code)
...
import main
import scratch.soft_rules_scaled as soft_rules_scaled

# Patch aplicar_reglas_blandas to use the scaled version
main.aplicar_reglas_blandas = soft_rules_scaled.aplicar_reglas_blandas
...
print(f"Solving with SCALED ratios (100) -> {status_map.get(status, 'UNKNOWN')}")
