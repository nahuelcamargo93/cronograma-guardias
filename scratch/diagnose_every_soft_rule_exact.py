...
def run_test(rule_to_disable):
    ...
    if rule_to_disable:
        ...
        content = content.replace(
            f"resolver_parametros_regla(\n            '{rule_to_disable}'",
            f"resolver_parametros_regla(\n            'DISABLED_RULE_{rule_to_disable}'"
        )
        # Handle MIN_DIA_ESPECIFICO_MES and EXACTO_DIA_ESPECIFICO_MES together
        ...
        if rule_to_disable == "FINDE_LARGO_REGLAMENTARIO":
            content = content.replace(
                "resolver_parametros_regla(\n            'FINDE_LARGO_REGLAMENTARIO_ESTRICTO'",
                "resolver_parametros_regla(\n            'DISABLED_RULE_FINDE_LARGO_REGLAMENTARIO_ESTRICTO'"
            )
            content = content.replace(
                "if active_flr_rule:",
                "if False:"
...
rules_to_test = [
    ...
    "PERSONAL_EXTRA_FUERA_MINIMO",
    "MIN_DIA_ESPECIFICO_MES"
]

print("=== DIAGNOSING EVERY EXACT SOFT RULE ===")
