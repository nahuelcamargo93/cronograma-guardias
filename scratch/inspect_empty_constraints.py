...
print("=== CHECKING FOR EMPTY BOOL_OR OR BOOL_AND CONSTRAINTS ===")
proto = modelo.Proto()
for c_idx, c in enumerate(proto.constraints):
    is_empty_or = c.bool_or and len(c.bool_or.literals) == 0
    is_empty_and = c.bool_and and len(c.bool_and.literals) == 0
    if is_empty_or or is_empty_and:
        print(f"Empty Constraint {c_idx} (name: '{c.name}'):")
        print(c)
        # Find which variables this constraint corresponds to (by checking if they are in enforcement_literal)
        if c.enforcement_literal:
            for lit in c.enforcement_literal:
                ...
                print(f"  Enforcement Var {var_idx}: {var_name}")
