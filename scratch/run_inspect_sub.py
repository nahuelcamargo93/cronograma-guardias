...
try:
    ...
    print("Running inspect_model_sub.py in subprocess...")
    res = subprocess.run(["python", "scratch/inspect_model_sub.py"], capture_output=True)
    stdout_decoded = res.stdout.decode("utf-8", errors="replace")
    stderr_decoded = res.stderr.decode("utf-8", errors="replace")
    
    # Write to output file
    with open("scratch/inspect_model_output.txt", "w", encoding="utf-8") as f_out:
        f_out.write("--- STDOUT ---\n")
        f_out.write(stdout_decoded)
        f_out.write("\n--- STDERR ---\n")
        f_out.write(stderr_decoded)
        
    print("Saved outputs to scratch/inspect_model_output.txt")
