...
def test_config():
    ...
    # MODIFY IN MEMORY:
    # 1. Set general MIN_HORAS_MES_CALENDARIO to 192
    reglas_servicio_db['MIN_HORAS_MES_CALENDARIO'] = {'min_horas': 192}
    
    # 2. Suspend/adjust for exceptions: Godoy, Barloa, Kolarik, Quintero, Motta
    exceptions = [
        'Godoy Maria',
    ...
    print("Testing with rules suspended for exceptions:")
    ...
    if feasible:
        print("[SUCCESS] The model is FEASIBLE with suspended min hours for these exceptions!")
    else:
        print("[FAIL] The model is INFEASIBLE even with suspended min hours for these exceptions.")
