...
# Build the exact same model with slacks on MIN_HORAS_MES_CALENDARIO
...
if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
    print(f"Optimal slack solved!")
    for (nombre, m_key) in [("ECHENIQUE ROCIO", "2026-07"), ("MIRANDA LUCIANA", "2026-07")]:
        info = slacks[(nombre, m_key)]
        print(f"\n=== SCHEDULE FOR {nombre} (Worked: {sum(solver.Value(v) for v in info['vars_h'])} hs, Slack: {solver.Value(info['slack'])} hs) ===")
        for d in range(31):
            day_str = (date(2026, 7, 1) + timedelta(days=d)).strftime("%a %d")
            assigned_shifts = [t for t in turnos_dict.keys() if (nombre, d, t) in turnos and solver.Value(turnos[(nombre, d, t)]) > 0]
            if assigned_shifts:
                print(f"  Day {day_str}: {assigned_shifts[0]} ({turnos_dict[assigned_shifts[0]].horas} hs)")
            elif d in [emp for emp in empleados if emp.nombre == nombre][0].dias_licencia:
                print(f"  Day {day_str}: [LICENCIA]")
