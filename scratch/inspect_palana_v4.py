...
try:
    ...
    for c_idx, c in enumerate(proto.constraints):
        ...
        if c.has_linear:
            ...
            for lit in c.bool_or.literals:
                ...
                if "PALANA" in var_names.get(v_idx, ''):
                    menciona = True
                    break
        elif c.has_exactly_one:
            tipo = "exactly_one"
            for lit in c.exactly_one.literals:
                v_idx = lit if lit >= 0 else -lit - 1
                if "PALANA" in var_names.get(v_idx, ''):
                    menciona = True
                    break
        elif c.has_at_most_one:
            tipo = "at_most_one"
            for lit in c.at_most_one.literals:
                v_idx = lit if lit >= 0 else -lit - 1
                if "PALANA" in var_names.get(v_idx, ''):
                    menciona = True
                    break
                    
        # También chequear variables de control (enforcement literals)
        ...
        if menciona:
            print(f"\nConstraint {c_idx} (name='{c.name}', type='{tipo}'):")
            print(c)
            if tipo == 'linear':
                terms = []
