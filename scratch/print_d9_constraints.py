...
print("=== CONSTRAINTS INVOLVING DIA 9 FOR CAMARGO ===")
proto = modelo.Proto()
for c_idx, c in enumerate(proto.constraints):
    ...
    if c.linear:
        for var_idx in c.linear.vars:
            ...
            if 'Camargo, Nahuel' in var_name and 'dia9' in var_name:
                vars_in_constraint.append(var_name)
    if c.bool_and:
    ...
    if vars_in_constraint:
        print(f"Constraint {c_idx} (name: '{c.name}', enforcement: {list(c.enforcement_literal)}):")
        print(c)
