...
TEMP_DB = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "temp_carolina_all.db")
...
def apply_fixes(db_path):
    ...
    # 4. Suspend hour limits for SUAREZ Carolina
    cursor.execute("""
        INSERT INTO personal_reglas_ajustes (personal_nombre, codigo_regla, fecha_inicio, fecha_fin, accion, parametros_json, activo)
        VALUES 
        ('SUAREZ Carolina', 'MIN_HORAS_MES_CALENDARIO', '2026-06-01', '2026-06-30', 'SUSPENDER', '{}', 1),
        ('SUAREZ Carolina', 'MAX_HORAS_MES_CALENDARIO', '2026-06-01', '2026-06-30', 'SUSPENDER', '{}', 1),
        ('SUAREZ Carolina', 'MAX_HORAS_SEMANA', '2026-06-01', '2026-06-30', 'SUSPENDER', '{}', 1)
    """)
...
def test():
    ...
    if is_feasible:
        print("[SUCCESS] The model has become FEASIBLE with Carolina's hour limits suspended!")
    else:
