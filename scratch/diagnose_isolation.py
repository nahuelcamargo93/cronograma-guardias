...
def run_isolated_test(rule_to_enable):
    ...
    # We always need LIMITES_SOFT_RULES as it is read at the beginning
    # and MAX_HORAS_SEMANA as it is required by hard rules validation
    ...
    elif rule_to_enable == 'MIN_FINDES_MES':
        reglas_servicio_db['MIN_FINDES_MES'] = db_rules.get('MIN_FINDES_MES', {})
    ...
    def patched_aplicar(*args, **kwargs):
        ...
        if rule_to_enable == 'SHIFT_EQUITY':
...
print("Running isolation diagnostics...")
for t in tests:
    run_isolated_test(t)
