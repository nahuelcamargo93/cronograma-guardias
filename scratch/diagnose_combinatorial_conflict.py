...
def build_full_model_except(disabled_rule_name):
    # Build base model
    ...
    # Apply DISTANCIA_MINIMA_TIPO_SEMANA
    ...
    # Force it (unless it is the disabled one, but DISTANCIA_MINIMA_TIPO_SEMANA is not in remaining_rules)
    ...
    for r_name in remaining_rules:
        ...
        # Force it to 1, EXCEPT if it is the target disabled rule
        for name, var in ctx.assumptions:
            if name == f"REG_{codigo}":
                if r_name == disabled_rule_name:
                    modelo.Add(var == 0) # Disable it!
                else:
                    modelo.Add(var == 1) # Enable it!
                break
                
    return modelo

print("=== PROBANDO DESACTIVAR REGLAS INDIVIDULAMENTE EN EL MODELO COMPLETO ===")
for r_name in remaining_rules:
    codigo = r_name.rsplit('.', 1)[-1].upper()
    modelo_temp = build_full_model_except(r_name)
    
    solver = cp_model.CpSolver()
    solver.parameters.max_time_in_seconds = 10 # 10s for solver
    status = solver.Solve(modelo_temp)
    status_name = status_names.get(status, "UNKNOWN")
    
    print(f"Desactivando solo {codigo}: {status_name}")
