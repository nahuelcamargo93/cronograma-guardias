...
TEMP_DB = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "temp_irma_no_susp.db")
...
def apply_fixes(db_path):
    ...
    # 3. Update EXCLUIR_TURNOS for BRIZUELA Irma to remove '18-24_Supervisor'
    ...
    # NOTE: We do NOT suspend Carolina's FINDES_COMPLETOS_Y_MEDIOS here!
...
def test():
    ...
    if is_feasible:
        print("[SUCCESS] Feasible WITHOUT suspending Carolina's weekend rule!")
    else:
        print("[FAIL] Infeasible WITHOUT suspending Carolina's weekend rule.")
