...
try:
    ...
    print("Total constraints:", len(proto.constraints))
    
    # Imprimir la constraint 49193
    target_idx = 49193
    if target_idx < len(proto.constraints):
        c = proto.constraints[target_idx]
        print(f"\nConstraint {target_idx} (name='{c.name}'):")
        print(c)
        if c.has_linear:
            terms = []
            for v_idx, coeff in zip(c.linear.vars, c.linear.coeffs):
                terms.append(f"{coeff}*{var_names.get(v_idx, f'var_{v_idx}')}")
            expr = " + ".join(terms)
            print(f"  Expr: {expr} in domain {c.linear.domain}")
    else:
        print(f"Target index {target_idx} out of range!")
